...
public class MediatorOperacao {
    ...
    public String realizarOperacao(boolean ehAcao, int entidadeCreditoId, int entidadeDebitoId, int idAcaoOuTitulo, double valor) throws IOException {
        ...
        if (ehAcao && !entidadeCredito.isgetAutorizadoAcao()) {
            return "Entidade de crédito não autorizada para ação";
        }

        if (ehAcao && !entidadeDebito.isgetAutorizadoAcao()) {
    ...
    public Transacao[] gerarExtrato(int entidadeId) throws IOException {
        Transacao[] transacoesCredito = repositorioTransacao.buscarPorEntidadeCredora(entidadeId);
        Transacao[] transacoesDebito = repositorioTransacao.buscarPorEntidadeDevedora(entidadeId);

        Transacao[] todasTransacoes = new Transacao[transacoesCredito.length + transacoesDebito.length];
        System.arraycopy(transacoesCredito, 0, todasTransacoes, 0, transacoesCredito.length);
        System.arraycopy(transacoesDebito, 0, todasTransacoes, transacoesCredito.length, transacoesDebito.length);

        java.util.Arrays.sort(todasTransacoes, (t1, t2) -> t2.getDataHoraOperacao().compareTo(t1.getDataHoraOperacao()));
